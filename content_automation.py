...
TEAM_NAME_MAP = {
    ...
    "Curaao":              "Curaao",  # Check if this exists in your data
...
WC2026_GROUPS = {
    ...
    "D": ["USA", "Paraguay", "Australia", "Trkiye"],

    "E": ["Germany", "Ecuador", "Ivory Coast", "Curaao"],
...
def generate_spanish_preview(home: str, away: str, a: dict) -> str:
    ...
    if a["top5_scorelines"]:
        ...
        score_line = f"MARCADOR MS PROBABLE: {home} {score_str} ({score_pct:.1f}%)"
...
def predict_wc2026_group(group_letter: str, verbose: bool = True) -> dict:
    ...
    for dt, rt in zip(display_teams, data_teams):
        ...
        if rt not in all_known:
            ...
            print(f"    {dt} ( {rt}) not found in dataset. Using defaults.")
