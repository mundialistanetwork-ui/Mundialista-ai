...
def get_team_points(team: str) -> int:
    ...
    if rankings.empty:
        return 1000
    row = rankings[rankings["country_full"] == team]
    return int(row.iloc[0]["elo"]) if len(row) > 0 else 1000


def ranking_factor(rank: int) -> float:
    """Log-scaled ranking advantage. Rank 1 â‰ˆ 1.53, Rank 50 â‰ˆ 1.14, Rank 200 â‰ˆ 1.0"""
    return 1.0 + 0.10 * math.log(201.0 / max(rank, 1))


def shrink_to_global(value: float, n_matches: int, global_mean: float) -> float:
    """Bayesian shrinkage â€” small samples pull toward global average."""
    k = CONFIG["SHRINK_K"]
...
def time_weight(match_date) -> float:
    """Exponential decay â€” recent matches matter more."""
...
def get_team_stats(team: str) -> dict:
    ...
    # â”€â”€ Combine home and away into unified match records â”€â”€
    ...
    # â”€â”€ Sort by date, take LAST N matches total â”€â”€
    ...
    # â”€â”€ Compute weighted averages â”€â”€
    weights = []
...
def compute_lambdas(team_a: str, team_b: str,
    ...
    # â”€â”€ Star player impact (relative, not absolute) â”€â”€
    ...
    # â”€â”€ Cap ratio for top team matchups â”€â”€
    if both_top:
...
def classify_match(rank_a: int, rank_b: int) -> str:
    ...
    if both_top and gap < 20:
        return "âš”ï¸ Elite Clash"
    elif gap > 100:
        return "ðŸ”» Total Mismatch"
    elif gap > 50:
        return "ðŸ“Š Clear Favorite"
    elif both_top:
        return "ðŸ”¥ Top Team Showdown"
    else:
        return "âš¡ Competitive Match"
...
def predict(team_a: str, team_b: str, home: str = None) -> dict:
    ...
    # â”€â”€ Top predicted scorelines â”€â”€
    score_probs = {}
    for i in range(max_g + 1):
        for j in range(max_g + 1):
    ...
    # â”€â”€ Monte Carlo (DC-consistent) â”€â”€
    n_sims = CONFIG["N_SIMULATIONS"]
    sim = simulate_matches(matrix, n_sims)

    # â”€â”€ Confidence check: analytical vs simulation â”€â”€
...
if __name__ == "__main__":
    ...
    print(f"  â”‚  Win: {result['team_a_win']:5.1f}%  â”‚  Draw: {result['draw']:5.1f}%  â”‚  Win: {result['team_b_win']:5.1f}%  â”‚")
